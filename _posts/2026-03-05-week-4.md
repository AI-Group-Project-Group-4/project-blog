...
* [Behaviour Cloning](#behavior-cloning)
* [PPO Model](#ppo-model)
...
With the PID driver working, we recorded it completing a number of laps so we could use a technique called Behavior Cloning. The idea is to give the RL agent a head start by showing it what good driving looks like, allowing us to skip the initial process of learning basic car control and move straight onto learning more complex behavior.


## PPO Model

Combining behavior cloning with a PPO model, we trained the agent for 1 million steps. As we weren't starting from scratch thanks to behavior cloning, this process was considerably sped up, allowing the model to learn the track through trial and error, earning rewards for clean driving and penalties for going off track. As training came to an end, the model had significantly outperformed the PID driver, discovering new racing lines to shave off as many seconds as possible.
